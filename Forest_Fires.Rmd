...
We did some initial EDA and visualization to get an idea of our next steps: the required data pre-processing.
...
## Outlier Detection
We see from the box plot below that there are 63 outliers (value outside 1.5 times the inter-quartile range) that may skew our results. These are removed, so that we are predicting burned areas of smaller to medium forest fires. This is reasonable, since these are the more frequently encountered ones.
...
5. ?zbayoglu, A. M., & Bozer, R. (2012). Estimation of the burned area in forest fires using computational intelligence techniques. Procedia Computer Science, 12, 282-287.
