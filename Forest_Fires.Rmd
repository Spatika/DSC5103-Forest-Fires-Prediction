...
```{r eval=TRUE, echo=TRUE, message=FALSE, tidy=TRUE}
model_lm = glm(area~ X+Y+rain+wind+temp+RH+area+DC+ISI+FFMC+month+day,
               data=data[train.index,])
summary(model_lm)
glm.diag.plots(model_lm)

sqrt(mean((data[train.index,'area']-model_lm$fitted.values)^2))

model_step = stepAIC(model_lm, direction = 'backward')
model_step$anova
sqrt(mean((data[train.index,'area']-model_step$fitted.values)^2))

model_lasso = cv.glmnet(area~.,
               data=data[train.index,], nfolds=10, use.model.frame = T, alpha = 1)

lam_min = model_lasso$lambda.min

train_pred = predict(model_lasso, s=lam_min, newx=data[train.index,], exact=TRUE)
...
tuned_rbf_pred_train = predict(tuned_rbf, train_data)
knn_pred_train = knn.reg(train_knn, train_knn, train_knn$X, k=k.opt)

stack_train <- cbind.data.frame(tuned_rbf_pred_train,knn_pred_train$pred,train_data$area)
stack_test <- cbind.data.frame(tuned_rbf_pred,knn_pred$pred, test_data$area)

names(stack_train) <- c("rbf","knn","area")
names(stack_test) <- c("rbf","knn","area")

manual_stack <- glm(area ~ .,data = stack_train)
manual_pred_stack <- predict(manual_stack, stack_test)

manual_stack_accuracy <- RMSE(manual_pred_stack, stack_test$area)
manual_stack_accuracy
