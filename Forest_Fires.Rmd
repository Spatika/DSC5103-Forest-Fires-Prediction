...
* So, based on the selected variables common to the tree and linear family, we find that the burned area is predominantly dependent on temperature, wind and relative humidity - these findings are in line with results in [1].
* However, additionally, we found DC to be an important predictor:
* The SVM and k-NN models show decent performance in terms of RMSE, but have lower interpretability.
...
models <- c("Linear Family (Ridge)", "k-NN", "SVM Linear", "SVM RBF", "RF", "XGBoost", "Ensemble - Manual Stacking")
rmses <- c(ridge_rmse, knn_rmse, svm_lin_tuned_rmse, svm_rbf_tuned_rmse, rf.rmse, xgb.rmse, manual_stack_rmse)
