...
## Normalization
For SVM and k-NN, we use normalized data on the 0-1 scale. Also for proper analysis and model comparison we will use normalized data for all models.
```{r eval=TRUE, echo=TRUE, message=FALSE, tidy=TRUE}
...
#shuffle data 
set.seed(17)
data_norm = data_norm[sample(1:nrow(data_norm)),]
...
rmse(test_pred_area_ridge, data[test.index,'area'])  
...
#Linear kernel
svm_linear = svm(area ~ ., data = data_norm[train.index,], kernel = 'linear', cost = 1)
...
tune_linear = tune(svm, area ~.,  data = data_norm[train.index,], kernel='linear',
                       ranges = list(epsilon = seq(0,1,0.1), cost = 2^(2:6), 
                       control = tune.control(sampling = "bootstrap", cross = 5, best.model = TRUE,
                                              fix = 2/3,nrepeat = 2)))
tuned_linear = tune_linear$best.model
tuned_linear_pred = predict(tuned_linear, data_norm[test.index]) 
tuned.pred.linear.denormalized = denormalize(tuned_linear_pred,min(data$area),max(data$area))

svm_lin_tuned_rmse = rmse(tuned.pred.linear.denormalized, data[test.index,'area'])
svm_lin_tuned_rmse # 


#RBF kernel
svm_rbf = svm(area ~ ., data=data_norm[train.index,], kernel = 'radial', cost = 1)
...
#Tuning RBF kernel
<<<<<<< HEAD
tune_rbf = tune(svm, area ~ .,  data = data_norm[train.index,], kernel='radial',
...
tune_rbf = tune(svm, area ~ rain + wind + temp + RH,  data = train_data, kernel='radial',
                 ranges = list(epsilon = seq(0,1,0.1), cost = 2^(2:8), control = tune.control(sampling = "bootstrap", cross = 5, best.model = TRUE, fix = 2/3,nrepeat = 2)))
>>>>>>> 8509d39393fe651f4432cbda72df34a4e59fc51f
tuned_rbf = tune_rbf$best.model
tuned_rbf_pred = predict(tuned_rbf, data[test.index,]) 
tuned.pred.rbf.denormalized = denormalize(tuned_rbf_pred,min(data$area),max(data$area))
svm_rbf_tuned_rmse = rmse(tuned.pred.rbf.denormalized, data[test.index,'area'])
svm_rbf_tuned_rmse #
...
rmse(pred.rf.train, data_norm[train.index,'area']) 
rmse(pred.rf, data_norm[test.index,'area'])
```
...
rmse(pred.xgb, data[test.index,'area']) 
```
