...
## Log Transformation
Since the data is still skewed towards very small fires, we take the log transformation as mentioned before to improve symmetry.
```{r eval=TRUE, echo=TRUE, message=FALSE, tidy=TRUE}
...
rmse(test_pred_area_lm, area_ha[test.index]) 
...
rmse(test_pred_area_lasso, area_ha[test.index])
...
rmse(test_pred_area_ridge, area_ha[test.index])  
...
knn_rmse = rmse(denormalize(test_knn$area, min(area_ha[test.index]), max(area_ha[test.index])), as.numeric(denormalize(knn_pred$pred,min(area_ha[test.index]), max(area_ha[test.index]))))
knn_rmse # 7.8
```
...
pred.linear.denormalized = as.numeric(denormalize(pred.linear,min(area_ha[test.index]), max(area_ha[test.index])))
svm_lin_rmse = rmse(denormalize(test_data$area, min(area_ha[test.index]), max(area_ha[test.index])), pred.linear.denormalized)
svm_lin_rmse # 9.34

>>>>>>> d07509be863c637b97ac08605efba38960626325
...
=======
tuned.pred.linear.denormalized = as.numeric(denormalize(tuned_linear_pred,min(area_ha[test.index]), max(area_ha[test.index])))
svm_lin_tuned_rmse = rmse(denormalize(test_data$area, min(area_ha[test.index]), max(area_ha[test.index])), pred.linear.denormalized)
>>>>>>> d07509be863c637b97ac08605efba38960626325
svm_lin_tuned_rmse # 
...
pred.rbf.denormalized = as.numeric(denormalize(pred.rbf,min(area_ha[test.index]), max(area_ha[test.index])))
svm_rbf_rmse = rmse(denormalize(test_data$area, min(area_ha[test.index]), max(area_ha[test.index])), pred.rbf.denormalized)
svm_rbf_rmse # 9.3
...
=======
tuned.pred.rbf.denormalized = as.numeric(denormalize(tuned_rbf_pred,min(area_ha[test.index]), max(area_ha[test.index])))
svm_rbf_tuned_rmse = rmse(denormalize(test_data$area, min(area_ha[test.index]), max(area_ha[test.index])), tuned.pred.rbf.denormalized)
>>>>>>> d07509be863c637b97ac08605efba38960626325
svm_rbf_tuned_rmse #

```
For both KNN and SVM we first tried predicting log_area, which turned out yielding generally low accuracies. We believe this is due to KNN and SVM being distance depended algorithms, and so ran the models without logarithmic transformation in the end.

For SVM, we also tried polynomial and sigmoid kernels, but came to conclusion that linear and RBF kernels outperform them with this data. Out of these models, KNN provides the lowest RMSE, with both SVM radial and linear kernels close behind. 
...
pred.rf.train <- predict(model.rf, newdata=data[train.index,]) 

# predict on test set
pred.rf <- predict(model.rf, newdata=data[test.index,]) 

# inverse log - 1
pred.rf.train <- exp(pred.rf.train) - 1
pred.rf <- exp(pred.rf) - 1

# rmse in train and test data
rmse(pred.rf.train, area_ha[train.index]) 
rmse(pred.rf, area_ha[test.index])
```
...
pred.xgb.train <- exp(pred.xgb.train) - 1

# rmse
rmse(pred.xgb.train, area_ha[train.index]) 
rmse(pred.xgb, area_ha[test.index]) 
```
...
* We test RMSE as our performance metric, we observe that both __ give us pretty good results as compared to the other models. Hence, we stack the above two models to augment our results. We find that the combination of these also gives a similar result.
* We notice that not all variables are significant in predicting the area (dependent variable) from the correlation plot. Hence we drop all the indices and RH, and instead use temperature.
...
# Insights
* The final model using the important variables produces a similar test RMSE to that observed when using all attributes. Hence, we can say that the burned area is indeed dependent on temperature, wind and rain. The other variables do not contribute much.
...
* As discussed, before this can be dealt with by labeling the datapoints by their severity (small, medium, big) and considering it as multi-class problem.
